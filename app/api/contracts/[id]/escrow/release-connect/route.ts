...
export async function POST(
  ...
  try {
    ...
    try {
      // Create transfer to freelancer's Connect account
      // This is the core of Stripe Connect's escrow functionality
      ...
      const { error: paymentError } = await supabase
        ...
        .insert({
          ...
          metadata: {
            ...
            released_by: user.id,
            reason: validatedData.reason || 'Payment release',
            payment_flow: 'stripe_connect_transfer',
          },
        });
      ...
      const { error: notificationError } = await supabase
        ...
        .insert({
          ...
          message: `Payment of $${releaseAmount.toFixed(2)} has been released to your account for contract ${contract.contract_number}`,
          metadata: {
            amount: releaseAmount,
            transfer_id: transfer.id,
            released_by: user.id,
            stripe_account: freelancerProfile.stripe_connect_account_id,
          },
        });
      ...
      return NextResponse.json({
        ...
        message: `Payment of $${releaseAmount.toFixed(2)} has been transferred to freelancer's account. Funds typically arrive within 1-2 business days.`,
      });
